...
import type { PaymentSourceType, PaymentStatus } from "@/types/payments";
  ...
  try {
    ...
    if (sourceType === "project" && !resolvedProjectId) {
      return NextResponse.json(
        {
          ok: false,
          error: "projectId is required when source type is project",
        },
        { status: 400 }
      );
    }
    ...
    if (sourceType === "requestForm" && !resolvedRequestFormId) {
      return NextResponse.json(
        {
          ok: false,
          error: "requestFormId is required when source type is requestForm",
        },
        { status: 400 }
      );
    }
    ...
    if (sourceType === "payroll" && !resolvedPayrollId) {
      return NextResponse.json(
        {
          ok: false,
          error: "payrollId is required when source type is payroll",
        },
        { status: 400 }
      );
    }
    ...
    let derivedFromRequestFormItems =
      payload.derivedFromRequestFormItems ?? false;
    let requestFormItemIds: string[] = [];
    let currency = payload.currency;

    if (sourceType === "requestForm" && resolvedRequestFormId) {
      const derived = await deriveItemsFromRequestForm(
        resolvedRequestFormId,
        payload.currency
      );
    ...
    requestFormItemIds =
      requestFormItemIds.length > 0
        ? requestFormItemIds
        : (normalizedItems.requestFormItemIds.filter(
    ...
    const totalAmount =
      payload.totalAmount ?? normalizedItems.totals.totalAmount;

    const installmentsInput = payload.installments
      ? normalizeInstallments(payload.installments)
      : [];
    ...
    const createdPayment = await prisma.payment.create({
      data: {
        ...
        isForeignCurrency:
          payload.exchangeRate != null ? payload.exchangeRate !== 1 : false,
        amount,
        taxAmount,
        totalAmount,
